...
# 使用 4 GPU (0,1,2,3), rule-based reward (不需要外部 Reward Model)
# 正常训练，输出 critic 模型供后续 freeze critic / reward mask 等实验加载
...
# 让 Ray worker 也能用 wandb（从本机 .netrc 读取的 key）
...
CRITIC_OUTPUT_PATH="${SCRIPT_DIR}/critic_qwen3_0.6b_ppo_math_lighteval"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3_0.6b_ppo_math_lighteval' \
    trainer.n_gpus_per_node=4 \
    trainer.nnodes=1 \
    trainer.save_freq=50 \
    trainer.test_freq=5 \
    trainer.total_epochs=3 \
    trainer.default_local_dir="${SCRIPT_DIR}/checkpoints/PPO-Project2026/qwen3_0.6b_ppo_math_lighteval" \
    +trainer.save_critic_model=True \
    +trainer.critic_model_output_path="$CRITIC_OUTPUT_PATH" $@

PYTHON_EXIT=$?

# 训练成功后，将 Critic（FSDP checkpoint 格式）导出到 CRITIC_OUTPUT_PATH，供你手动转为 HF 后给 freeze critic 等实验用
if [ $PYTHON_EXIT -eq 0 ]; then
    CKPT_DIR="${SCRIPT_DIR}/checkpoints/PPO-Project2026/qwen3_0.6b_ppo_math_lighteval"
    if [ -d "$CKPT_DIR" ]; then
        ...
        if [ -n "$LATEST" ] && [ -d "${LATEST}/critic" ]; then
            ...
            echo "Critic 导出完成。转为 HF 后使用，例如:"
            echo "  python scripts/convert_critic_to_hf.py --local_dir $CRITIC_OUTPUT_PATH --target_dir ${CRITIC_OUTPUT_PATH}_hf --trust-remote-code"
