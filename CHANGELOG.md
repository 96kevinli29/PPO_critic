...
- **Reward Mask**: 约第 1565-1602 行，搜索 `"========== Reward Mask"`
- **Reward Mask Advantage 翻转**: 约第 1632-1649 行，搜索 `"Reward Mask: 翻转被 mask 轨迹"`
  - 当 reward 被 mask 为 0 时，GAE 得到的 advantage 符号相反；对**被 mask 的轨迹**翻转 advantage，使 critic value 作为奖励代理（高 V→鼓励）
- **Freeze Critic 跳过更新**: 约第 1653 行，搜索 `"# update critic (skip if frozen)"`
...
| `+algorithm.reward_mask_flip_adv_when_masked` | 对被 mask 轨迹的 advantage 取反（用 critic value 作奖励代理） | true |
